...
static bool pairing_in_progress = false;  // Track if pairing is ongoing
...
void  send_light_notification(float lux)
{
    uint8_t lux_buffer[2];
    lux_buffer[0] = (uint8_t)(lux & 0xFF);            // Low byte
    lux_buffer[1] = (uint8_t)((lux >> 8) & 0xFF);     // High byte

    // Step 1: Write to local GATT attribute
    sl_bt_gatt_server_write_attribute_value(
        gattdb_light_intensity,
        0,
        sizeof(lux_buffer),
        lux_buffer
    );

    // Step 2: Enqueue to send Indication
    write_queue(gattdb_light_intensity, lux_buffer, sizeof(lux_buffer));

    // Step 3: Try to send immediately if possible
    process_indications();

    // Step 4: Print to LCD if allowed
    if (bleDataPtr->connection_open && bleDataPtr->ok_to_send_htm_indications) {
        displayPrintf(DISPLAY_ROW_TEMPVALUE, "Lux: %d", lux);
    }
        ...
        case sl_bt_evt_gatt_server_characteristic_status_id:
            if (evt->data.evt_gatt_server_characteristic_status.status_flags == sl_bt_gatt_server_client_config) {
                if (evt->data.evt_gatt_server_characteristic_status.client_config_flags == sl_bt_gatt_indication) {
                    ...
                    if (evt->data.evt_gatt_server_characteristic_status.characteristic == gattdb_light_intensity) {
...
switch (SL_BT_MSG_ID(evt->header))
{
  case sl_bt_evt_system_boot_id:
    ...
    sc = sl_bt_connection_set_default_parameters(
        60,  // Minimum connection interval (in units of 1.25ms) → 60 * 1.25ms = 75ms
        60,  // Maximum connection interval (in units of 1.25ms) → 60 * 1.25ms = 75ms
        4,   // Slave latency → Number of connection intervals a slave can skip if no data is exchanged
        83,  // Supervision timeout (in units of 10ms) → 83 * 10ms = 830ms/10
        0,   // Minimum CE length (unused, typically 0)
        4    // Maximum CE length (unused, typically set to 4)
    );
      ...
      sc = sl_bt_scanner_set_parameters(
          1,   // Scan mode: 1 → Passive scanning
          80,  // Scan interval
          40   // Scan window
      );
      if (sc != SL_STATUS_OK) {
  ...
  // Checking if the discovered service is Health Thermometer (0x1809)
    if (evt->data.evt_gatt_service.uuid.len == 2) {  // UUID should be 16-bit (2 bytes)
      uint16_t service_uuid = (evt->data.evt_gatt_service.uuid.data[1] << 8) |
                               evt->data.evt_gatt_service.uuid.data[0];

     if (service_uuid == HEALTH_THERMOMETER_SERVICE_UUID) {
  ...
  case sl_bt_evt_system_external_signal_id:
      // 🔹 Step 3: If PB1 was pressed, attempt to read `button_state`
      if (evt->data.evt_system_external_signal.extsignals & PB1_EVENT_PRESSED) {
          if (!pairing_in_progress) {
              sl_bt_gatt_read_characteristic_value(ble_data.connection_handle, gattdb_button_state);
          }
      }

      // 🔹 Step 4: If PB0 is pressed for pairing confirmation
      if ((evt->data.evt_system_external_signal.extsignals & PB0_EVENT_PRESSED) && waiting_for_passkey_confirmation) {
  ...
  case sl_bt_evt_gatt_procedure_completed_id:
      // 🔹 Step 5: If read fails with encryption error, start bonding
  ...
  case sl_bt_evt_sm_confirm_passkey_id:
      // 🔹 Step 6: Display passkey and wait for PB0 confirmation
      pairing_passkey = evt->data.evt_sm_confirm_passkey.passkey;
      displayPrintf(DISPLAY_ROW_PASSKEY, "Passkey: %06lu", pairing_passkey);
      displayPrintf(DISPLAY_ROW_ACTION, "Confirm with PB0");
      waiting_for_passkey_confirmation = true;
  ...
  case sl_bt_evt_sm_bonded_id:
      // 🔹 Step 7: Bonding successful, retry reading `button_state`
  ...
  case sl_bt_evt_gatt_characteristic_id:
     if (evt->data.evt_gatt_characteristic.uuid.len == 2) {  // UUID should be 16-bit (2 bytes)
        uint16_t char_uuid = (evt->data.evt_gatt_characteristic.uuid.data[1] << 8) |
                              evt->data.evt_gatt_characteristic.uuid.data[0];

     if (char_uuid == TEMPERATURE_MEASUREMENT_CHAR_UUID) {
