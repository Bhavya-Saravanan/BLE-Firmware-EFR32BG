...
#define TIMER_PERIOD (32769) //for 1khz ticks
#define LAZY_SET       (0)   // A value of 0 means the timer starts without delay.
#define TIMER_IDENTIFY  (0)    // Identifier for this specific timer instance.
#define SINGLE_SHORT_VALUE (0)   // 0 means the timer will continue running periodically.
 ...
 *
 * This function configures the LCD, initializes the GLIB graphics context,
 * and starts a Bluetooth stack soft timer to update the EXTCOMIN pin.
 *
 * @param None
 * @return None
 */
void displayInit()
{
    ...
    //
    si7021_power_control(true); //power on the lcd
    ...
    sl_status_t timer_response = sl_bt_system_set_lazy_soft_timer(TIMER_PERIOD, LAZY_SET, TIMER_IDENTIFY, SINGLE_SHORT_VALUE);
        if (timer_response != SL_STATUS_OK) {
            LOG_ERROR("LCD UPDATING THE SOFT TIMER IS NOT SET CORRECTLY: 0x%x", timer_response);
        }
 ...
 */
/**
 * @brief Updates the LCD EXTCOMIN pin.
 *
 * This function is called periodically by the Bluetooth stack soft timer
 * to toggle the EXTCOMIN pin and prevent charge buildup on the LCD.
 *
 * @param None
 * @return None
 */
void displayUpdate()
{
  ...
  // Students: Create the function gpioSetDisplayExtcomin() that will set
	...
	// Calling gpioSetDisplayExtcomin() to update EXTCOMIN pin
