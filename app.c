...
#define APP_IS_OK_TO_SLEEP      ((LOWEST_ENERGY_MODE == 0)? false :true)
...
SL_WEAK void app_init(void)
{
  ...
  oscillator_init();// Initialized the oscillator to configure the clock source.



  if (LOWEST_ENERGY_MODE == 1)// Setting the power manager requirements based on the lowest energy mode
...
{
    /*static state_t currentState = STATE_IDLE; // Current state of the state machine
    uint32_t evt = getNextEvent(); // Fetch the next pending event

    // Process the event using the state machine
    handle_state_machine(&currentState, evt);*/
}
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  handle_ble_event(evt); //responders for the event
  #if DEVICE_IS_BLE_SERVER
  // SERVER
  // sequence through states driven by events
  state_machine_project(evt);    //state transition to get the temperature value
  #else
   //CLIENT
